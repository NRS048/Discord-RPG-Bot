...
async def begin(ctx):
    ...
    try:
        ...
        await ctx.send(f"what is your character's name?")
        msg = await bot.wait_for("message", timeout=5)
        name = msg.content
        await ctx.send(f"what is your character's race?")
        await ctx.send(f"human, elf, troll")
        msg = await bot.wait_for("message", check=race_check, timeout=5)
        race = msg.content.lower()
        await ctx.send(f"what is your character's gender?")
        await ctx.send(f"male, female, other")
        msg = await bot.wait_for("message", check=gender_check, timeout=5)
        gender = msg.content.lower()

        await ctx.send(f"add charachter stat points, must add upp to 50")
        await ctx.send(f"stealth")
        msg = await bot.wait_for("message", timeout=5)
        stealth = int(msg.content)
        await ctx.send(f"strength")
        msg = await bot.wait_for("message", timeout=5)
        strength = int(msg.content)
        await ctx.send(f"intelligence")
        msg = await bot.wait_for("message", timeout=5)
        intelligence = int(msg.content)
        await ctx.send(f"dexterity")
        msg = await bot.wait_for("message", timeout=5)
